...
[trajectory_global,inputs_global] = find_global_path(goal,A_global,B_global,C,Q_matrix_global,R_matrix_translation,obstacles,max_linear_velocity/10,max_linear_force/2,max_linear_force_difference, horizon_global,treshold);

% %% Prepare Translation MPC
% % MPC translation parameters
% Ts_rotation=Ts_translation/10;
% horizon_translation=20;
% treshold_translation=0.1;
% Q_scalar_translation=20;
% R_scalar_translation=0.1;
% max_distance_from_waypoint=1.5;%max_linear_velocity*Ts_global;
...
%         If system is close to the next waypoint, 
    while norm(x_estimated_translation(1:3)-current_waypoint(1:3))<(max_distance_from_waypoint/2)
    ...
    %         constraints_beq_translation,[],[],[],options);
        ...
        inputs=quadprog(Hes,grad,constraints_translation_A,b_constraints_tranlsation,...
            constraints_Aeq_translation,constraints_beq_translation,[],[],[],options);
        if isempty(inputs)
            interpolation=true;
